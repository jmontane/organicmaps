...
UNIT_TEST(GetTtsStreetTextTest)
{
  string const engShortJson =
      ...
      \"make_a_right_turn\":\"Make a right turn.\",\
      \"make_a_left_turn\":\"Make a left turn.\",\
      \"dist_direction_onto_street\":\"%1$s %2$s onto %3$s\",\
      \"you_have_reached_the_destination\":\"You have reached the destination.\"\
      }";
  ...
  string const jaShortJson =
      ...
      \"dist_direction_onto_street\":\"%1$s%2$s %3$s に入ります\",\
  ...
  string const faShortJson =
      ...
      \"dist_direction_onto_street\":\"%1$s %2$s ﺐﻫ %3$s\",\
  ...
  TEST_EQUAL(getTtsText.GetTurnNotification(notification4), "その先 左折です。", ()); // note the extraneous space here due to + " " +
  ...
  TEST_EQUAL(getTtsText.GetTurnNotification(notification4), "ﺲﭙﺳ ﺐﻫ ﭻﭘ ﺐﭙﯿﭽﯾﺩ.", ()); // note the extraneous space here due to + " " +
}
